...
from ..db import db
from ..models import Player, Item, PlayerAchievement, PhaseProgress, PlayerItem, Achievement, Battle, ResultType
...
def ranking():
    ...
    players = Player.query.order_by(Player.experience.desc(), Player.level.desc()).all()
    return jsonify([
        {
            'rank': index + 1,
            'id': player.id,
...
def update_player(id: int):
    ...
    player.saved_at = datetime.utcnow()
    db.session.commit()
    return jsonify({
...
def player_achievements(id: int):
    validate_access(id) # Garante que apenas o próprio usuário ou um admin possa ver
    Player.query.get_or_404(id)
...
# Demais rotas preexistentes (phases, etc.) continuam abaixo...
